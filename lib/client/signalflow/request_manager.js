...
function RequestManager(options) {
  ...
  function registerLiveTail(params, callbacks, overrideRequestId){
    var requestId = overrideRequestId || getRequestIdAndIncrement();
    knownComputations[requestId] = {
    ...
    knownComputations[requestId].streamController = getLiveTailStreamController();
  ...
  function getLiveTailStreamController() {
    return {
      stop: stopLiveTail,
      retry: registerLiveTail
    };
  }
