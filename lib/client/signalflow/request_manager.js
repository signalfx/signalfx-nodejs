...
function RequestManager(endPoint) {
  ...
  function addComputation(params, requestId, transportType, cb, requestType, isRetryPatchMode) {
    var router = routedMessageHandler(params, cb, isRetryPatchMode);
  ...
  function streamComputationWebsocket(params, requestId, cb, requestType, isRetryPatchMode) {
    //hack, obviously
    if (!wsConnectionOpen) {
      pendingWSRequests.push({params: params, requestId: requestId, callback: cb, requestType: requestType, isRetryPatchMode: isRetryPatchMode});
    } else {
