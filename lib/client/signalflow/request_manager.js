...
function RequestManager(endPoint) {
  ...
  function getJobObject(params) {
    requiredParams.forEach(function (reqParam){
      if(!params[reqParam]) {
        throw new Error('Unable to create a program without required property:'+reqParam);
      }
    });
    var obj = {};
  ...
  function routeMessage(msg, requestId) {
    ...
    } else if (!removedComputations[requestId]) {
      console.error('A message was received for an unknown requestId : '+requestId);
    }
  }
  ...
  function removeComputation(requestId) {
    if (knownComputations[requestId]) {
      ...
      removedComputations[requestId] = true;
      return true;
    } else {
      return false;
    }
