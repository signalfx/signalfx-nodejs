...
function RequestManager(options) {
  ...
  function registerLiveTail(opts, callbacks, overrideRequestId ){
    var requestId = overrideRequestId || getRequestIdAndIncrement();
    addComputation(opts, requestId, transports.WEBSOCKET, callbacks, requestType, isRetryPatchMode);
    var jobObject = getJobObject(opts, requestType);
    jobObject.type = requestType;
    jobObject.channel = requestId;
    activeSocket.send(JSON.stringify(jobObject));
